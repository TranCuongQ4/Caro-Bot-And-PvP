var board = {
    ...
    sqUpdate: function(i, j) {
        var OHtml = '<img src="o.png">';
        var XHtml = '<img src="x.png">';
    ...
    sqMouseOver: function(row, col) {
        if (currGame.isGamming && currGame.sq[row][col] == 0) {
            var HTML = '<img style="opacity: 0.5" src="' + (currGame.Turn == X ? 'x.png' : 'o.png') + '">';
            document.getElementById('s' + String('00' + row).slice(-2) + String('00' + col).slice(-2) + '').innerHTML = HTML;
        }
    },
    /* khi click vào ô trên bàn cờ */
    sqClick: function(row, col) {
        if (currGame.isGamming && currGame.sq[row][col] == 0) {
            currGame.makeMove(row, col); // Gọi hàm makeMove để xử lý nước đi
        }
    },
};