...
function game(noOfRow, noOfCol) {
    ...
    this.xMove = function(i, j) {
        if (this.isGamming && this.sq[i][j] == 0 && this.Turn == X) {
            this.sq[i][j] = X;
            board.sqUpdate(i, j);
            this.noOfPiece++;
            this.moveHistory.push({row: i, col: j, value: X}); // Lưu nước đi của người chơi
            this.Turn = O;
            referee.checkWin();
            if (this.isGamming == false) {
                return;
            }
            var bestMove = {row: 0, col: 0};
            AIthink(O, bestMove);
            this.sq[bestMove.row][bestMove.col] = O;
            board.sqUpdate(bestMove.row, bestMove.col);
            this.noOfPiece++;
            this.moveHistory.push({row: bestMove.row, col: bestMove.col, value: O}); // Lưu nước đi của Bot
            this.Turn = X;
            referee.checkWin();
        }
    };
